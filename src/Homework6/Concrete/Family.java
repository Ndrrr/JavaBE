...
import Homework6.Abstract.HumanCreator;
...
//    }
    ...
    public void deleteChild(AbstractHuman child){
        for (int i = 0; i < this.children.length; i++) {
    ...
    public AbstractHuman bornChild() {
        ...
        if(gender){
            return new Man(Utils.getRandomName('m',false),
            ...
            return new Woman(Utils.getRandomName('f',false),
                    this.father.getSurname(),
                    LocalDate.now().getYear(),
                    (this.father.getIq()+this.mother.getIq())/2,
                    this);
